...
// `oculto` tira o modelo da ESCOLHA sem apagá-lo: peça que já usa segue renderizando e
// exibindo o nome certo. É o caso dos três foto-reais, que o Hugo reprovou.
...
const MEDIA_MODELS = [
  ...
  { id: "foto_maos_mesa", name: "Foto real (mãos + mesa)", grupo: "Foto real", oculto: true, desc: "Mãos e caderno sobre mesa escura. É a cena mais sóbria das três.", svg: '<svg viewBox="0 0 40 40" width="30" height="30" fill="none" stroke="currentColor" stroke-width="2"><rect x="8" y="12" width="24" height="16" rx="2"/><path d="M6 16v10M34 16v10"/><circle cx="31" cy="8" r="3"/></svg>' },
      ...
      <div class="card">
        ...
        <div class="form-section media-only" id="g-media-section" style="display:none">
          ...
          <div class="field"><label>Modelo do dispositivo <span class="hint">(como o print aparece na arte)</span></label>
            <div id="g-media-model-pick">${MEDIA_GRUPOS.filter((g) => MEDIA_MODELS.some((m) => m.grupo === g.nome && !m.oculto)).map((g) => `
              <div class="model-grupo">
                <div class="model-grupo-tit">${esc(g.nome)} <span class="hint">— ${esc(g.desc)}</span></div>
                <div class="model-pick">${MEDIA_MODELS.filter((m) => m.grupo === g.nome && !m.oculto).map((m) => `
